...
void parallel_execution_native::map(
  ...
  MPI_File_open(MPI_COMM_WORLD, 'Fileinput', MPI_MODE_RDONLY, MPI_INFO_NULL, &fh);

  //Different chunck size must be specified per process.
  const int chunk_size = sequence_size / concurrency_degree_;
  //Take into account overhead in chunck distribution. Last processes could end up reading too much.
  ...
  MPI_File_set_size(fhout, sequence_size*sizeof(auto));
  MPI_File_seek(fhout, rank*chunk_size, MPI_SEEK_SET);

  for (int i = 0; i < chunk_size; i++) 
  {
  	//Input data type?
  	MPI_File_read(fh, aux, 1, MPI_INT, MPI_STATUS_IGNORE);

  	//Apply transform op
  	aux = apply_deref_increment(
          std::forward<Transformer>('transform_op'), aux);
  ...
  {
    const auto l = next(get<0>(fins), chunck_size);
    while (get<0>(fins)!=l) {
